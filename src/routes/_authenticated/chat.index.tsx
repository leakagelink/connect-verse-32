...
function ChatList() {
  ...
  return (
    <AppShell isAdmin={me?.isAdmin}>
      <div className="mb-6"><h1 className="text-2xl font-bold">Chats</h1></div>
      {isLoading ? (
        ...
        <div className="space-y-2">
          {convs.map((c) => (
            <button key={c.id} onClick={() => navigate({ to: "/chat/$conversationId", params: { conversationId: c.id } })} className="w-full text-left">
