 ...
 */
export function SmartAvatar({ userId, url, username, gender, aiStyle, className, alt }: Props) {
  const [failed, setFailed] = React.useState(false);
  const resolvedSrc = React.useMemo(() => {
    if (url && !failed) return url;
    if (userId) return aiAvatarUrl(userId, aiStyle, gender);
    return undefined;
  }, [url, userId, aiStyle, gender, failed]);
